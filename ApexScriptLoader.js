...
function init()
{
	...
	params = getProvidedArguments();
	
	if(params.length == 0)
	{
		log('No config file provided. Provide one by specifying the folder name that contains your config.json file in the scripts directory of this application"');
	}
	else
	{
		scriptName = params[0];
	...
	log('Read configurations from scripts\\' + scriptName + '\config.json\r\n');
	if(configData.applicationName) log('Application Name: ' + configData.applicationName + '\r\n');
	if(configData.author) log('Author: ' + configData.author + '\r\n');
	if(configData.description) log('Description: ' + configData.description + '\r\n');
	if(configData.createdDate) log('Created Date: ' + configData.createdDate + '\r\n');
	if(configData.apiVersion) log('For API Version: ' + configData.apiVersion + '\r\n');
	var d = new Date();
	d.toLocaleString();  
	
	log('Started process at ' + d, false);
	
	log('Calling Apex script. Please wait...');
	
	injectVariables();
	
	
	//call to the function which will call the apex script
	runScript(configData.apexScriptFile,configData.apexResultFile,configData.username,function(error, stdout, stderr){
		
		//if an error is detected, log all available data
		if(error || stderr)
		{
			log('Error Detected During Apex Script Execution',true,'red');
			log(JSON.stringify(stderr, null, 2),true,'red');
			log(JSON.stringify(error, null, 2),true,'red');
			log(JSON.stringify(stderr, null, 2),true,'red');
		...
		fs.readFile('scripts/'+scriptName+'/'+configData.apexResultFile, 'utf8', function (err,data) {
			...
			var logData = parseLog(data);
			
			if(!logData.success)
			{
				log('Error Detected in Apex Script or During Parsing!',true,'red');
				log(logData.message,true,'red');
			}
			else
			{
				log('Script ran successfully',true,'green');						
			}
			for(i = 0; i<logData.logLines.length; i++)
			{	
				log(logData.logLines[i]), true;
			}
		});	
	});
...
function runScript(apexScriptFileName,apexResultFileName,username,callback){
...
function readConfigs(scriptName)
{
	...
	{
		log('Error reading configuration file scripts/'+scriptName+'/config.json Please check it exists and is properly formatted JSON.',true,'red');
